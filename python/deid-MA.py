# edited by Wenjing Ma on catching PHI HCPName
# still, I feel the comments are elegent and easy to comprehend, thus I just re-used most of them

import re
import sys

# refering to the deid.pl about HCPName information
...
def check_for_HCPName(patient, note, chunk, output_handle):
    ...
    Inputs:
        ...
        - output_handle: an opened file handle. The results will be written to this file.
            ...
            to this function. 
    Logic:
        Search the entire chunk for HCPName occurances. Find the HCPName of these occurances 
        ...
        Use prefixes to check for HCPNames as follow.
    """
    ...
    # search the whole chunck and find every position that matches the location_indicators, 
    # then, write the word after the indicator to the output_handle
    checked_HCPName = [] # searched names in this chunck
...
def deid_HCPName(text_path = 'id.text', output_path = 'location.phi'):
    """
    Inputs: 
        - text_path: path to the file containing patient records
        - output_path: path to the output file, default: HCPName.phi
    ...
    with open(output_path,'w+') as output_file:
        with open(text_path) as text:
            ...
            for line in text:
                ...
                if len(record_end):
                    # find all HCPName in the record chunk
